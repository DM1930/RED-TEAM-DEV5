...
#pragma endregion
	...
	void Update_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		...
		if (registry.all_of< VulkanVertexBuffer, VulkanIndexBuffer>(entity))
		{
			...
			VulkanGPUInstanceBuffer instBuffer;
			...
			for (auto entity : instances) {
				const auto& geoData = instances.get<GeometryData>(entity);
				const auto& gpuInstance = instances.get<GPUInstance>(entity);

				gpuInstanceList.push_back(gpuInstance);

				if (geometryMap.find(geoData) != geometryMap.end()) {
					geometryMap[geoData]++;
				}
				else {
					geometryMap[geoData] = 1;
				}
			}

			registry.emplace<std::vector<GPUInstance>>(entity, gpuInstanceList);
			...
			for (const auto& [geometry, instanceCount] : geometryMap)
			{
				vkCmdDrawIndexed(
					commandBuffer,
					geometry.indexCount,     
